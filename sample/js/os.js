...
(function( $ ) {
	...
	$.fn.winify = function(options)
	{
		...
		for(i=0;i<length;i++)
		{
			...
			for(k=1;k<=lim;k++)
			{
				obj.append("<div> item " +k +": lorem ipsum is lorem ipsum and lorem ipsum</div>");
			}
    ...
    function doanimation(obj,chlength,settings)
	{
		...
		nextchild.css("z-index","3");
		nextchild.css("top","0");
		
		//current child will be there behind next child reduce its opacity in 1s
		currentchild.animate({opacity:0},1000,function(){
			currentchild.animate({top:settings.height},200,function(){
				nextchild.css("z-index","2");
